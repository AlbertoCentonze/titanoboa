...
def generate_bytecode_for_arbitrary_stmt(
    ...
    """Generate bytecode for an arbitrary stmt by compiling a wrapper source.

    Performs type inference for expressions when ``return_type`` is not
    provided, using the VyperContract's namespace.
    """

    # Attempt to infer return type for expressions if not explicitly provided
...
def generate_source_for_arbitrary_stmt(
    ...
    - If ``return_type`` is provided, treat ``source_code`` as an expression
      and emit a wrapper that returns that type.
    - Otherwise, emit a statement wrapper without a return.
    """
